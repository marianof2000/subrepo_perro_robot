...
>
> Si trabajás con el repositorio completo, podés seguir [la guía con uv](#usar-el-repositorio-con-uv).
>
...
## Usar el repositorio con uv

`uv` administra Python, las dependencias y el entorno virtual del proyecto.
El repositorio usa **Python 3.12** (ver `.python-version`) y guarda las versiones
de las dependencias en `uv.lock`. El workspace actual incluye la raíz y **TP02**;
para los requisitos específicos de otros laboratorios, consultá su `INSTALACION.md`.
...
Cerrá y volvé a abrir la terminal, y comprobá con `uv --version`.
También hay otras opciones en la [guía oficial de instalación de uv](https://docs.astral.sh/uv/getting-started/installation/).

### 2. Prepará el entorno

Abrí una terminal en la **raíz del repositorio**, donde están este README,
`pyproject.toml` y `uv.lock`, y ejecutá:
...
`uv` descarga Python si hace falta y crea `.venv` con las dependencias,
incluido MuJoCo. `--locked` comprueba que la configuración coincida con el
lockfile sin actualizarlo. Necesitás internet para la descarga inicial.
La raíz y TP02 comparten el entorno y el lockfile según el
[funcionamiento de los workspaces de uv](https://docs.astral.sh/uv/concepts/projects/workspaces/).

En Windows, si aparece `DLL load failed`, seguí el paso del **Visual C++
Redistributable** en la instalación manual de abajo.

### 3. Abrí el simulador del TP02

Desde la raíz, en la **primera terminal**, ejecutá en Linux o Windows:

```bash
uv run --directory 05LaboratoriosTPs/TP02_Programacion_I/entorno python -m sim --robot go2 --materia tp02
```

**En macOS**, usá `mjpython` para la ventana 3D, como indica la
[documentación de MuJoCo](https://mujoco.readthedocs.io/en/stable/python.html#passive-viewer):

```bash
uv run --directory 05LaboratoriosTPs/TP02_Programacion_I/entorno mjpython -m sim --robot go2 --materia tp02
```

`go2` selecciona el perro; cambiá ese valor por `g1` para el humanoide.
Esperá a que aparezca el robot y dejá el simulador abierto.

### 4. Ejecutá tu programa

Abrí una **segunda terminal en la raíz del repositorio** y ejecutá:

```bash
uv run python 05LaboratoriosTPs/TP02_Programacion_I/mi_desarrollo/mi_tp02.py
```

Editá `mi_tp02.py`, guardalo y repetí ese comando cada vez que quieras probar.
Con `uv run` no necesitás activar `.venv`. Los lanzadores por doble clic buscan
otros intérpretes de Python; para usar el entorno de uv, seguí los comandos de
esta sección.

Después de actualizar el repositorio, volvé a ejecutar `uv sync --locked`.

---

## Instalación manual (sin uv)
